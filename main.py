...
def image1():
	if request.method=='POST':
		...
		f = request.files['doc']
		f.save(os.path.join(savepath,(secure_filename('test.jpg'))))
		return redirect(url_for('image_test1'))
	return render_template('image1.html',name=name)
...
def image_test():
	...
	return render_template('image_test.html',name=name,	result=result)

@app.route('/image_test1', methods=['GET', 'POST'])
def image_test1():
	gtype = ''
	result,index = predictQuality()

	return render_template('image_test1.html',name=name,	result=result)
